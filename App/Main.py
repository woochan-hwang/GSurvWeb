...
def main(dev_mode=False, path_to_local_data='data/example_data_template.xlsx', verbose=False):
    ...
    # Load file
    if dev_mode and path_to_local_data != None:
    ...
    if upload_status:
        ...
        if dev_mode:
            app_mode = st.sidebar.selectbox('Select mode', ['Data Summary', 'Interactive', 'Experiment', 'Develop'], key='app_mode',
                                            on_change=reset_session_state(verbose=verbose))
        else:
            app_mode = st.sidebar.selectbox('Select mode', ['Data Summary', 'Interactive', 'Experiment'], key='app_mode',
                                            on_change=reset_session_state(verbose=verbose))
        # run selected app mode                                    
        if app_mode == 'Data Summary':
            data_summary(file, verbose=verbose)
        elif app_mode == 'Interactive':
            interactive_run(file, verbose=verbose)
        elif app_mode == 'Experiment':
            experiment(file, verbose=verbose)
...
if __name__ == '__main__':
    ...
    if args.develop:
        from main.dev import dev
        main(dev_mode=True, path_to_local_data=args.path_to_data, verbose=args.verbose)
    else:
        assert args.path_to_data==None, 'please use --develop option to specify path to local data'
